...
### 10. Post-visual stabilization follow-up (agent-executable)

Agent A (platform/docs lane):
...
Execution gates:
1. `A5-DOC-SYNC` and `B6-KEYBOARD-COVERAGE` run in parallel (`Batch N1`).
2. `A6-INPUT-GUARDS` and `B7-SPACE-UX` run in parallel after `Batch N1` (`Batch N2`).
3. Final acceptance gate after both batches: `bun test tests`, `bun run typecheck`, `bun run build`.
